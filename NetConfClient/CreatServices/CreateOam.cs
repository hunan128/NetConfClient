    ...
    class CreateOAM
    {
        ...
        public static XmlDocument State(string _ctp_name, string _dm_state, string _tm_state, string _lm_state, string _cc_state,string ISP)
        {
            ...
            if (ISP.Contains("联通"))
            {
                ...
                ctp.AppendChild(eth_ctp_pac);
                //OAMconfig
                XmlElement oam_state_pac = commonXml.CreateElement("oam-enable-pac");
                eth_ctp_pac.AppendChild(oam_state_pac);
                //MEP-ID
                XmlElement dm_state = commonXml.CreateElement("dm-enable");
                dm_state.InnerText = _dm_state;
                oam_state_pac.AppendChild(dm_state);
                //远端MEP-ID
                XmlElement tm_state = commonXml.CreateElement("tm-enable");
                tm_state.InnerText = _tm_state;
                oam_state_pac.AppendChild(tm_state);
                //MGE-ID
                XmlElement lm_state = commonXml.CreateElement("lm-enable");
                lm_state.InnerText = _lm_state;
                oam_state_pac.AppendChild(lm_state);
                //md-name
                XmlElement cc_state = commonXml.CreateElement("cc-enable");
                cc_state.InnerText = _cc_state;
                oam_state_pac.AppendChild(cc_state);
            }
            if (ISP.Contains("电信"))
